...
def create_grid(spatial_resolution):
    ...
    meshgrid = torch.meshgrid(
        *[torch.linspace(-1.0, 1.0, d_i) for d_i in spatial_resolution], indexing="ij"
    )
...
grid_samples = {2: custom_grid_sample2d, 3: custom_grid_sample3d}
...
class SelfWarp(nn.Module):
    ...
    def __init__(
        ...
        self.flow_head = nn.Sequential(
            ...
            conv_modules[self.n_spatial_dims](
                out_channels, self.n_spatial_dims * self.num_heads, kernel_size=1
            ),
        )
        self.value_head = conv_modules[self.n_spatial_dims](
    ...
    def forward(self, u, meta=None):
        ...
        flow = rearrange(
            flow,
            flow_fold_heads[self.n_spatial_dims],
            dir=self.n_spatial_dims,
        ...
        value = rearrange(
            value, value_fold_heads[self.n_spatial_dims], heads=self.num_heads
        )
        ...
        # unfold the heads
        u_warp = rearrange(
            u_warp, value_unfold_heads[self.n_spatial_dims], heads=self.num_heads
        )
    ...
    def __init__(
        ...
        if use_meta_conditioning:
            ...
            self.norm = nn.GroupNorm(
                num_groups=groups, num_channels=out_channels, affine=True
            )
...
class Flower(nn.Module):
    ...
    def __init__(
        ...
        self.project = nn.Sequential(
            conv_modules[self.n_spatial_dims](out_ch, lifting_dim, kernel_size=1),
            nn.ReLU(inplace=True),
            conv_modules[self.n_spatial_dims](lifting_dim, dim_out, kernel_size=1),
        )
        self.dropout = (
            nn.Identity() if dropout_rate == 0.0 else nn.Dropout(p=dropout_rate)
        )
